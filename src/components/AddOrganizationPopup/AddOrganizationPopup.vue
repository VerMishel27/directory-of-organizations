							...
							<div class="form__data">
									...
									<label class="form__label">
											<input name="number"
														 ...
														 v-model.number="organizationNumber"
														 class="form__input">
									</label>
									<label class="form__label">
											<input name="nameDirector"
														 required
														 type="text"
														 minlength="2"
														 maxlength="30"
														 placeholder="ФИО директора"
														 v-model="organizationDirector"
														 class="form__input">
									</label>
							</div>
							<div class="form__block-button">
									<button class="form__button form__button_cancel"
													@click="closeAddPopup()"
													type="button">
											Отмена
									</button>
									<button :disabled='disableOk'
													class="form__button form__button_submit"
													:class="{ form__button_disable: disableOk }"
													type="submit">
											Ок
									</button>
							</div>
...
<script>
	...
	export default {
		...
		submitForm(event) {
			...
			computed: {
					disableOk() {
							return !this.organizationName || !this.organizationNumber || !this.organizationDirector;
					}
			}
	}
</script>